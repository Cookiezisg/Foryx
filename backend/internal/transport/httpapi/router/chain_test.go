...
func TestChain_MuxErrorsEnveloped(t *testing.T) {
	...
	h := Chain(mux, zap.NewNop(), nil) // /api/v1/workspaces* is exempt → no 401 first

	// 404: unknown route under /api/v1/* → N1 envelope, not "404 page not found" plaintext.
	w := httptest.NewRecorder()
	h.ServeHTTP(w, httptest.NewRequest("GET", "/api/v1/workspaces/does-not-exist", nil))
	...
	h.ServeHTTP(w2, httptest.NewRequest("DELETE", "/api/v1/workspaces", nil))
	...
	h.ServeHTTP(w3, httptest.NewRequest("GET", "/nope", nil))
