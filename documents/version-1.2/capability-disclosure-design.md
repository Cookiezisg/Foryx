...
| 组成 | ~token | 占比 |
|---|---|---|
| 用户消息 + system prompt 静态段(base/multi_agent/locale) | ~430 | 1.5% |
...
```text
<section name="base">                  身份  ~25t  [静态·可缓存]
<section name="tool_conventions">       三个标准字段讲一次  ~120t  [静态·可缓存]
<section name="capabilities">           统一能力菜单(对象层)  ~200t–1k  [半动态]
<section name="multi_agent_forging">    Subagent 策略(精简)  ~150t  [静态·可缓存]
<section name="memory">                 长期记忆  0–?  [动态]
<section name="user_systemPrompt">      对话级(可选)  [动态]
<section name="locale_hint">            语言  ~20t  [动态]
```

`tool_conventions` 与 `base` 排在 `capabilities` 之前,使「base + tool_conventions + tools 参数」构成稳定可缓存前缀(§4.5)。

**完整示例(中文便于阅读;实现时用英文省 token)**:
...
- `destructive` 壳：`{"type":"boolean","default":false,"description":"true if this call may be irreversible; see tool_conventions."}` — 保留字段以解析调用结果；long guidance 移至 `tool_conventions` 段。
- `execution_group` 壳：`{"type":"integer","minimum":1,"description":"Parallel-batch id; see tool_conventions."}` — 同上；long guidance 移至 `tool_conventions` 段。
...
- 稳定可缓存前缀 = `base` + `tool_conventions` + `tools` 参数(常驻集)。这些跨请求不变。
