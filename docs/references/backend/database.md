...
| `sandbox_runtimes` | runtime manifest（全机解释器/镜像：kind+version · path · size_bytes，`UNIQUE(kind,version)`）——**系统级**（无 ws 列、owner 无关）、**硬删**（盘上镜像是实体，墓碑无意义） |
| `sandbox_envs` | env manifest（owner kind+id · runtime · status）——**系统级**（owner-id 全局隔离、无 ws 列）、**硬删**（盘上目录是实体，墓碑无意义） |
