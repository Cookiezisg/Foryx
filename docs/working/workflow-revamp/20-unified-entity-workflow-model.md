...
>
> **校准状态（R0047 前 · 2026-06-08）**：本文已对齐**落地代码**——`FieldSpec`→`schema.Field{Name,Type,Description}`（无 Required/integer）；`InputSchema`→`Inputs`；`Branch.Port` **保留**（直接对接 `Edge.FromPort`，不叫 Enum）；ctl/apf **无 `Outputs`**（输出派生：ctl=各分支 emit 的 keys、apf=常量 `{decision,reason}`）；agent input = 结构化 JSON 喂 LLM（非模板插值）；mcp 加 `schema.FromJSONSchema` 派生 Field。ctl/apf 回炉 + 全实体 I/O 统一**已完成并 push**（contract-changes #27/#28）。承重墙（一切数据皆某祖先 result + `ctx.runId`，边不搬数据）+ 循环回边可见性（`BackEdges()` 校验与解释器共用纯函数）经 study 确认无变。
...
实体 = {
  ...
  Inputs  : []schema.Field       // 我要哪几个输入——每个实体都声明（:run/:invoke 表单 + workflow 接线都要这张入参清单）
  逻辑                            // input → output 的具体实现（各实体不同）
  版本: 单调版本号 + active 指针   // 同 function；pin 所必需（在途 run 不漂移）
}

// 输出怎么声明，看实体能否自由产任意字段（对称的是「都声明 Inputs」，不是「都塞 Outputs」）：
//   fn / hd.method / ag / trg → 显式 Outputs []schema.Field（返回是黑盒，不声明没人知道）
//   ctl                       → 无 Outputs：输出 = 各 Branch.Emit 的 keys（per-port，比并集准）
//   apf                       → 无 Outputs：恒为常量 {decision, reason}
//   mcp tool                  → 存 server JSON Schema，wiring 时 schema.FromJSONSchema 派生 Field 视图

type Field struct {              // internal/pkg/schema —— 全锻造实体共享的唯一 I/O 字段类型
    Name        string
    Type        string  // string | number | boolean | object | array（刻意无 integer / required / default / 嵌套——精确塑形交运行时 CEL）
    Description string
}
```
...
- **引用**：`ag_`。
- **input**：节点 `Input` map（接线）求值出的结构数据，**整坨 JSON 贴在 agent 自己的 prompt 后**喂 LLM（**非**模板插值——对 LLM 比插值稳）。agent 声明 `Inputs []schema.Field` 当入参清单（:invoke 表单 + 接线用）。
- **output**：agent 声明的 `Outputs []schema.Field`——非空 ⇒ 被指示以含这些字段的 JSON object 作答；空 ⇒ 自由文本。
- **出口**：单。
- **落地（R0043 + I/O 统一已完成）**：已是 `Inputs`/`Outputs []schema.Field`；input→task 的 JSON 拼接已在 `invoke.go`。workflow 轮只需在 scheduler 把节点 Input 求值后喂进 `InvokeInput.Input`。
...
type ControlVersion struct {  // ctlv_
    Version  int
    Inputs   []schema.Field   // 判定输入（如 score, attempt）；when/emit 读 input.*
    Branches []Branch         // 有序：first-true-wins，末条 When="true" 兜底
    // 无 Outputs 字段：输出 = 各 Branch.Emit 的 keys（per-port，下游按 port 读对应分支）
}
type Branch struct {
    Port string             // 具名结局：workflow 据此路由（= 边的 FromPort）——保留 Port，不叫 Enum
...
- **output**：选中行的 `(Port, Emit 求值后的数据)`。**Port 驱动路由**、**Emit 数据是本节点 result**（下游按名读 `gate.feedback`）。无独立 outputs 声明——emit 的 keys 即输出字段。
- **出口**：N 个具名结局（`Port`），**只激活选中的那一个**（XOR）。**workflow 负责「Port → 连去哪个节点」，control 完全不知道下游节点**。
- **能力边界**：when/emit 仅确定性 CEL（读 input、禁 now()、禁外部）——不是轻量 function，不做任意计算。
- **落地（R0045 + 回炉 R0047① 已完成 + push）**：`when`/`emit` 读 `input.*`；`Version` 有 `Inputs`（DB 列 `inputs`）；**`Branch.Port` 保留**（直接对接 `Edge.FromPort`，不叫 Enum）；**无 `Outputs`**（输出=emit keys，per-port 校验比并集准）。`Resolve(ref, pin版本)→[]Branch` 给 scheduler 求值。
...
type ApprovalVersion struct {  // apfv_
    Version         int
    Inputs          []schema.Field  // 我展示哪些字段（如 amount, payee）；template 读 input.*
    ...
    // 无 Outputs 字段：输出恒为常量 {decision, reason}
}
```
...
- **落地（R0046 + 回炉 R0047② 已完成 + push）**：`template` 读 `{{ input.* }}`；`Version` 有 `Inputs`（DB 列 `inputs`）；output 恒为 `{decision, reason}`，**无 `Outputs` 字段**（常量不存）。`Resolve(ref, pin版本)→*Version` 给 scheduler。
...
| 东西 | 现状 | 改动 | 轮次 |
|---|---|---|---|
| **fn_** | `Inputs`/`Outputs` | **无**（范本） | ✅ |
| **hd_.method** | `Inputs`/`Outputs` | **无** | ✅ |
| **mcp tool** | server JSON Schema | + `schema.FromJSONSchema` 派生 Field（未接入 mcp，wiring 轮用） | ✅ |
| **ag_**（R0043） | `Inputs`/`Outputs` | input→task JSON 在 invoke；scheduler 轮接喂 input | ✅ |
| **trg_**（R0039） | `Outputs`(payload) | payload=result | ✅ |
| **ctl_**（R0045） | `Inputs`+Port+emit | 回炉完成：读 `input.*`、Port 保留、无 Outputs（§5.4） | ✅ R0047① |
| **apf_**（R0046） | `Inputs`+template | 回炉完成：读 `input.*`、无 Outputs（常量，§5.5） | ✅ R0047② |
| **workflow** | — | **新建**（§6 全套，relation 第 12 类 `wf_`） | ← 待建 R0047 |

**好消息**：fn/hd/mcp/ag/trg 这 5 个调用类实体**天生符合**（fn 是范本）。最后建的两个新实体 ctl_/apf_ 已回炉——根因是当初让它们直读 scope、绑死了图；改成读 input 后它俩就和 fn_ 一样 workflow-agnostic、可复用。

**顺序**（每步 verify + commit + push）：
1. ✅ 本文（doc 20）定稿 + 校准到落地现实。
2. ✅ **ctl_ 回炉**（R0047①）：`Inputs` + Port 保留 + 删 Outputs；全栈 + 契约。
3. ✅ **apf_ 回炉**（R0047②）：`Inputs` + 删 Outputs；全栈 + 契约。
4. ← **workflow 新建**（R0047，下一步）：domain → store → app（CRUD + ops + 校验 + BuildPinClosure + WorkflowReader）→ tool（7 个，无 :trigger/执行类）→ handler → relation 第 12 类 → 测试 → 契约。ag_/trg_ 轻确认并入此轮。
