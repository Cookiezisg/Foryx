...
case 的执行语义是**纯控制流、无副作用**:逐分支求 `when:` 布尔,**第一个 true 的中选、往下走**,**其余分支根本不执行**。`when:` 只读 payload / ctx(= 已记账的结果),因此**对确定性重放天然友好**(详 00 的"确定性"段)。

**分支重新汇合(菱形 merge,A-1)**:若多条 case 分支汇到同一下游节点,该汇合是 **active-branch join(WP5 simple-merge)**——**只等被激活的那条**。机制:case 中选一条后,对**未走的分支下发 skip 信号**,顺图传到汇合点;汇合节点 `await 被激活的入边`,据此**不等死没来的分支**。这跟 AND-split 扇出的汇合(等全部,WP3)是**两种 join**,由 split 类型(XOR vs AND)决定 —— 详 [`00`](./00-overview.md) 并发模型段。**不写成 "await 全部入边" 否则菱形会死锁。**
...
- **不嵌套结构化循环(V1,C6)** — 一个循环体内不再含另一个循环头,`iteration_key` 因此保持**一维**(节点 + 第几轮)。要嵌套(外层遍历 N 项、内层每项重试)就把内层做成 **forge 工具 / function**(对位"map over N → 工具":动态扇出 / 逐项迭代塞进锻造的能力内部并发)。**不是 sub-workflow** —— workflow 不能被当 callable 引用(员工思维,见 [`00`](./00-overview.md) §3 / [`09`](./09-agent-domain.md));所以唯一逃生舱是 forge 工具(A-7)。accept 校验拒嵌套回边。
