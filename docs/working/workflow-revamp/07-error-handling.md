...
解释器查该节点的 retry policy(maxAttempts / backoff)
    ...
    ↓ 用尽 → 把"此 activity 永久失败"记进事件日志 → flowrun 标 `failed` + 推通知
      (注:trigger 失败不在这条 flowrun-activity 路上 —— trigger 是入口非 activity,
       失败在 listener / 收件箱层,见下"Trigger 特例",A-3)
```
...
入口失败是不同性质——它**不在 flowrun 内**(trigger 是程序入口、不是 activity,见 [`01`](./01-triggers.md)),失败发生在 **listener / 收件箱 / pre-flowrun polling-function 层**,且 workflow 客观上不能继续工作:

```
trigger 在 listener 层失败(cron 注册失败 / webhook handler 报错 / polling function 崩)
    ↓ (还没进 flowrun;在 trigger_schedules / 收件箱层按其 retry 配置重试,
       不是 flowrun 内 activity retry —— retry 配置落在 trigger_schedules,不在节点 inspector)
    ↓ N 次后仍失败
    ↓
...
| **Cancel 机制(WP19/20)** | `cancel_flowrun`:写一条 `flowrun_cancelled` journal 事件 → 解释器 goroutine 收 `ctx.Done` 停、在途 activity 收 ctx 取消、parked 的 approval / timer 标 cancelled;flowrun 标 `cancelled`。取消是**整 flowrun 粒度**(单节点取消不单列) |
...
| 策略 | 怎么实现 |
|---|---|
| **失败重试次数 / backoff** | **tool 节点** config 填 `retry: {maxAttempts, backoff}`;不填 = 0 次,失败立即通知。**agent 节点取默认、无 retry 旋钮(要自定义就用 tool 节点调 `ag_`,见 02,A-4);trigger 的 retry 在 listener 层 / `trigger_schedules`,不在节点 config(A-3)** |
...
| 重跑 flowrun | `POST /flowruns/{id}:replay` | 带现有事件日志重放(语义见上;语义由用户/AI 决定要不要跑)。**replay-reset 精确契约(写 `replay_started` → `generation++` → 终态/失败步只看最高代 → record-once UNIQUE 含 generation)见 [`17`](./17-execution-contract.md) §4** |
...
```
1. retry 次数                   → **tool 节点** config(用户/AI 拍,不填=0;agent 取默认无旋钮;trigger 的 retry 在 listener 层 —— A-3/A-4)
