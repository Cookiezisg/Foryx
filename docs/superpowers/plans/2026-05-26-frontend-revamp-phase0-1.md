...
```
- [ ] **Step 2: 装 eslint + boundaries + steiger**
```bash
...
- [ ] **Step 1: 写 `frontend/tsconfig.json`**
```json
{
  "compilerOptions": {
    "target": "ES2022",
...
- [ ] **Step 2: 写 `frontend/tsconfig.node.json`(给 vite/eslint config 自身)**
```json
{
...
- [ ] **Step 1: 写 `frontend/eslint.config.js`**
```js
...
"fsd": "steiger src"
```
- [ ] **Step 2: Makefile 加 `lint-frontend` 目标**
```makefile
lint-frontend:
...
- [ ] **Step 1: 建 `shared/api/httpClient.ts`** —— 把 `api/client.js` 的 `apiFetch` / `ApiError` / `pickList` / `EMPTY_ARRAY` 搬来,加类型:
```ts
...
- [ ] **Step 2: `api/client.js` 改 re-export shim**
```js
...
- [ ] **Step 4: commit**(精确 add httpClient.ts + client.js)

### Task 1.2:`shared/api/queryKeys.ts`(qk 工厂 + 类型)

**Files:** Create `frontend/src/shared/api/queryKeys.ts`

- [ ] Step 1: 把 `client.js` 的 `qk` 工厂搬来,加 `as const` + 返回类型(`readonly [string, ...]`)。
- [ ] Step 2: client.js shim 已转发(Task 1.1 Step 2)。
- [ ] Step 3: vitest + tsc 绿。Step 4: commit。
...
- [ ] Step 1: 搬 `sse/shared.js` 的 `createSSE`,加类型(`{ path, eventHandlers, onStatus }` 接口、`SSEController`)。**不改自愈逻辑**(阶段 4 改为 signalAuthFailure)。
- [ ] Step 2: `sse/shared.js` → `export * from "@shared/api/sse"`。
- [ ] Step 3: vitest + tsc 绿。Step 4: commit。
...
- [ ] Step 1: 建集中表(对位后端 `errmap.go`),把已知 error code 映射到默认用户文案 key(i18n)。先放骨架 + 已知 code(UNAUTH_NO_USER / CONVERSATION_NOT_FOUND / 通用 fallback);后续 feature 用例按需补。**此 task 只建表,不接全局 onError**(那在阶段 3/4 收口)。
- [ ] Step 2: tsc 绿。Step 3: commit。
...
- [ ] Step 1: 搬 `bridge/wails.js`(`initBaseUrl`/`apiUrl`/`GetBackendPort` 调用),加类型。Wails runtime 调用处加最小类型声明(无 wailsjs 生成目录,手写 `declare` 即可)。
- [ ] Step 2: `bridge/wails.js` → re-export。
- [ ] Step 3: vitest + tsc + **`make dev` 冒烟**(bridge 是 Wails 接触面,必须确认壳能拿到 backend port)。Step 4: commit。
...
- [ ] Step 1: 逐个 primitive `.jsx → shared/ui/*.tsx`,定 props 接口。`Select.tsx` 是之前做的统一 Select,props 已清晰,直接标类型。
- [ ] Step 2: `shared/ui/index.ts` barrel re-export 全部。
- [ ] Step 3: 旧 `components/primitives/*` 各改 re-export shim(指向 `@shared/ui/*`)。
- [ ] Step 4: vitest(primitives 的 .test + 用到它们的组件测试)全绿 + tsc。Step 5: commit。
...
- [ ] Step 1: `motion/tokens.js → shared/lib/motion.ts`(定型 token 对象),旧路径 re-export。
- [ ] Step 2: i18n 装配移进 `shared/lib/i18n/`(locales 保持),更新 `main.jsx` 的 `import "./i18n"` → `@shared/lib/i18n`;旧 `i18n/index.js` re-export。
- [ ] Step 3: vitest(i18n key 完整性测试等)全绿 + tsc。Step 4: commit。

### Task 1.8:对 `shared/` 开启 boundaries 强制 + 引入 steiger

**Files:** Modify `frontend/eslint.config.js`、Create `frontend/steiger.config.js`

- [ ] Step 1: eslint boundaries 加正式 `shared` element(`src/shared/**`),规则:`shared` 只能 import `shared`(违规升 `error`)。其余层仍 warn(还没建)。
- [ ] Step 2: 建 `steiger.config.js`(@feature-sliced/steiger-plugin),先只校验已存在的 `shared` slice 的 public-api / no-orphan。
- [ ] Step 3: Makefile `lint-frontend` 加 `npm run fsd`。
- [ ] Step 4: `make lint-frontend` —— `shared` 层零违规(它只依赖自身);若有 shared 内文件引了上层,修。
- [ ] Step 5: commit。

### Task 1.9:阶段 1 收口验证 + push

- [ ] Step 1: `cd frontend && npx vitest run && npm run build && npx tsc --noEmit && npm run lint && npm run fsd`
Expected: vitest 全绿、build 成功、tsc 过、lint(shared error 级)零违规、steiger(shared)零违规。
- [ ] Step 2: `make dev` 冒烟(壳 + 后端连通)。
- [ ] Step 3: 确认所有旧路径 shim 透明(grep 没有断裂 import)。commit + push。
