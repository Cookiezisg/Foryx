...
- **波次0 地基**：M0.1 pkg ✅（**reqctx/idgen/pagination ✅** R0001；**tokencount ✅** R0002；**pathguard ✅** R0003；**userpath ⏭️删** R0004；**wikilink ✅** R0005；**jsonrepair ✅** R0006；**limits ✅** R0007；modelcaps/modelcatalog 移交 M1.3）· M0.2 db ⬜ · M0.3 logger/crypto ⬜ · M0.4 errors/eventlog/notif(domain) ⬜ · M0.5 infra eventlog/notif/chat ⬜ · M0.6 llm ⬜ · M0.7 transport 框架 ⬜
...
- **M0.2**：`infra/db`（modernc sqlite + 迁移；schema 可激进重定）。M0.1 纯工具全完；`modelcaps`/`modelcatalog` 移交 M1.3 model 判定（model 数据层，非通用工具）。
