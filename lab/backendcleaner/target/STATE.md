...
- **波次0 地基**：M0.1 pkg 🔧（**reqctx/idgen/pagination ✅** R0001；**tokencount ✅** R0002；**pathguard ✅** R0003；**userpath ⏭️删** R0004；**wikilink ✅** R0005；**jsonrepair ✅** R0006；余 limits + `modelcaps` 残留判定）· M0.2 db ⬜ · M0.3 logger/crypto ⬜ · M0.4 errors/eventlog/notif(domain) ⬜ · M0.5 infra eventlog/notif/chat ⬜ · M0.6 llm ⬜ · M0.7 transport 框架 ⬜
...
- **M0.1 续**：`limits` 考古（M0.1 最后一个纯工具）→ 判定 `modelcaps`（modelcatalog 取代→大概率删）/`modelcatalog` 残留。然后 M0.2 `infra/db`。
