...
> 状态：✅ 已拍板（2026-06-03 定稿；2026-06-04 修订：Node 简化为通用 `{Type,Content}`，砍三流 domain 包）。
...
## 3. Node —— 通用 payload（domain 不持词表）

```go
type Node struct {
    Type    string          `json:"type"`              // 自由字符串，可编码层级："tool_call" / "tool_call/read_file"
    Content json.RawMessage `json:"content,omitempty"` // 该 type 携带的任意 JSON；domain 从不解析
}
```

**协议刻意不枚举 node 类型或其字段。** 灵活 ≠ `map[string]any`，但也 ≠ domain 替业务穷举判别联合——而是 domain 只给一个 `{Type, Content}` 信封，**词表的定义权下放给各 producer 业务模块**（chat 定义它发的 message 节点类型，function/agent 的 producer 定义 entities 的）。

为什么不在 domain 定词表：domain 根本不知道未来 UI 要哪些 node、各带什么字段（这也是它曾被标 PROVISIONAL 的根源）。与其"定一个暂定的"，不如**根本不定**——更诚实，且让 domain 永不因加一种 node 而改动。符合 #6 反校验剧场（domain 不校验 content 内部）、#7 零包袱。

**type 常量下沉到使用它的业务模块**（哪个业务发，常量登记在哪），domain 不持词表。下表**仅示意、非契约**，各 producer 轮定：

| 流 | 示例 type（业务侧定义/登记） |
|---|---|
| **messages** | `message` · `text` · `reasoning` · `tool_call` · `tool_result` · `progress` · `compaction` |
| **entities** | `forge` · `run` · `env_attempt` · `terminal` · `message`(复用——agent 运行对话双输出) |
| **notifications** | `entity_changed` · `flowrun_tick` · `flowrun_lifecycle` |

> message 也是一种 node（`type=message`）——**一切皆节点**，无 message/block 双层；message 是顶层节点，text/tool_call 挂其下，tool_call 调的 agent 对话又是挂在 tool_call 下的 message 节点。全递归。content 怎么用（哪个字段是流式内容、delta 怎么拼进去）由业务定。
...
`ListReader` = Bridge + REST 快照拉取；仅 notifications 流用（无 DB 落盘，List 读内存 buffer）：
...
**无三流各自的 domain 包**：node 通用、Bridge 通用，messages/entities/notifications 不再有 domain 包。三条流的区别全部落到 **infra 实例 + scope kind + wiring**：单一 `Bus` 实例化三次，messages/entities 消费方按 `Bridge` 注入、notifications 按 `ListReader` 注入（同一 Bus 两者皆满足，区别在按哪个接口接线）。

校验 `ValidateEvent(Event) error` 在 `domain/stream`：scope.kind 合法、节点 id 非空、frame 内部一致（open/signal 的 node 须有 Type、close 须有终态）；**不碰 Node.Content**（业务的事）。
...
7. **Node 通用化（domain 不持词表）**〔2026-06-04 修订〕：`Node` 从判别联合改为通用 `{Type, Content}`；node 词表下放各 producer 业务；domain 永不因加 node 改动。连带**砍掉 messages/entities/notifications 三个 domain 包**，只留 `domain/stream`（Bridge + ListReader）。理由：domain 不该猜它不知道的词表（反校验剧场 + 零包袱）。
8. **error 内聚 domain/errors**〔2026-06-04〕：stream 的 `ErrSeqTooOld`(KindGone→410) / `ErrInvalidEvent`(KindInternal) 用结构化 domain 错误而非标准库 sentinel——会上 HTTP 的错误经 statusForKind 零特例（R0012 方针）；连带 `domain/errors` 加 `KindGone`(410)。
...
- **type 常量下沉**：旧三流 node 词表常量（text/tool_call/forge/...）→ 各 producer 业务模块自己定义登记，domain 不持词表。
